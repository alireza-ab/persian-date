...
const PersianDate = function (date, calendar) {
	...
	const getDayLabel = (date = new Date(), calendar = "jalali") => {
		if (Object.prototype.toString.call(date) === "[object Date]")
			// if the year was an instance of Date
			return _utils_js__WEBPACK_IMPORTED_MODULE_0__["CALENDAR"][calendar]["days"]["label"][
				date.getDay()
			];
	...
	const getDayOfWeek = (
		...
		if (Object.prototype.toString.call(date) === "[object Date]") {
			// if the year was an instance of Date
			return (
				_utils_js__WEBPACK_IMPORTED_MODULE_0__["CALENDAR"][calendar]["days"]["weekNumber"][
				date.getDay()
				] + (mode != "standard" ? 0 : 1)
			);
		}
	...
	PersianDate.prototype.toString = function (format = "date") {
		...
		format = format.replace(
			_utils_js__WEBPACK_IMPORTED_MODULE_0__["REGEX"]["betweenBacktick"],
			(matched, text) => {
				texts.push(text);
				return "###";
			}
	...
	PersianDate.prototype.month = function (format = "") {
		...
		if (_utils_js__WEBPACK_IMPORTED_MODULE_0__["REGEX"]["isNumeric"].test(format)) {
			...
			if (format == "MMM")
				return _utils_js__WEBPACK_IMPORTED_MODULE_0__["CALENDAR"]["gregorian"]["months"][
					gMonth
				].slice(0, 3);
			return month;
		}
	...
	const typesToArray = function (
		...
		if (
			typeof year == "string" &&
			year.search(_utils_js__WEBPACK_IMPORTED_MODULE_0__["REGEX"]["separators"]) != -1
		) {
